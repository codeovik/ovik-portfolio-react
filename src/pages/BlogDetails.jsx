...
const BlogDetails = () => {
  ...
  if (!blog) return (
    <div className="h-dvh flex items-center justify-center flex-col">
      ...
      <h2 className='text-2xl md:text-4xl font-medium mt-50 dark:text-white'>Loading. Please wait.</h2>
    </div>
  );
    ...
    <>
      <header className='overflow-hidden h-400 relative top-0 flex items-center justify-center flex-col px-20 md:px-40 gap-20'>
        ...
        <p className="dark:text-white md:text-xl text-xs font-medium flex gap-5 items-center">
          ...
          <svg className='scale-60 fill-black dark:fill-white' xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e3e3e3"><path d="m321-80-71-71 329-329-329-329 71-71 400 400L321-80Z"/></svg>
          <Link to='' className='text-primary'>{blog.title}</Link>
        </p>
      ...
      <section className="max-w-[1400px] mx-auto px-20 md:px-40 text-black dark:text-white">
        ...
        <div className="grid gap-30 lg:grid-cols-[1fr_400px]">
          ...
          <div className="flex flex-col gap-30">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-30">
              ...
              <div className="bg-lightbox dark:bg-darkbox p-20 rounded-2xl md:rounded-3xl">
                ...
                <div className="grid gap-y-15">
                  {Object.entries(blog.details).map(([key, value]) => (
            ...
            {/* recent */}
            <div className="bg-lightbox dark:bg-darkbox p-20 rounded-2xl md:rounded-3xl">
              <h3 className="text-3xl font-medium mb-10">Recent blogs:</h3>
              <div className="grid md:grid-cols-2 lg:grid-cols-1 gap-10">
                {recentBlog.map((e, index) => (
                  <Link
                    to={`/blog/${e.title.toLowerCase().replace(/\s+/g, '-')}`}
                    key={index}
                    className="grid grid-cols-[30%_70%] gap-10 p-10 border-3 md:border-3 border-white/10 hover:border-primary/50 transition-all rounded-xl"
                  >
                    <img
                      loading="lazy"
                      src={e.imageLink}
